<?php
    ...
    function updateUserdata($user_id,$combined,$connection){
        $updateUserDetails="UPDATE users set ";
            ...
            $firstIterations=true;
            foreach($combined as $fields => $values){
                

                $valueString = ($values === null || $values === NULL || $values === 'NULL' || $values === "null") ? "NULL" : "'" . $values . "'";

                if($firstIterations){
                    $updateUserDetails.=" ".$fields."=".$valueString."";
    ...
    function removeUserFromfac($user_id_arr,$facility_id,$connection){
            ...
            if($usrDatares==="Not Exist"){
                ...
                if($checkRes===1){     
                    if($_SESSION['user_id']===$user_id){
                        ...
                        // Check if the type of user which need to be removed is not FADMIn if yes dont not allow 
                        $usrDataresFADMIN=getUserData("userID",$user_id,$connection,"type");                                                
                        if($usrDataresFADMIN!="Not Exist"){                            
                            if($usrDataresFADMIN==="FADMIN" || $usrDataresFADMIN==="SADMIN"){
                                $return_res.="<li>Error: You cannot Remove Facility Admin from facility ".$usrDatares."</li>";
                            }
                            else{
                                ...
                                if($updateUserFac===1){                                    
                                    ...
                                    $return_res.="<li>Error: Cannot Client/Customer removed from facility ".$usrDatares."</li>";
                                }  

                            }
                        }
                        else{
                            $return_res.="<li>Error: while removing Client/Customer ".$usrDatares."</li>";
                        }
                    ...
                    //Remove user from facility hence null/update 0 in fac_id iin users table for specific user entry
                    $return_res.="<li>Error: Client/Customer not under your facility ".$usrDatares."</li>";
                }
                else if($checkRes===-1){
                    $return_res.="<li>Error: while removing Client/Customer ".$usrDatares."</li>";
                }
        ...
        if($count>0){
            $return_res.="<li>Error: ".$count." Client/Customer Ids not found</li>";
        }
        $return_res.="</ul>";        
        return $return_res;
    }
    ...
    if(!isset($_SESSION['isSession'])){
        ...
        $checkPer=checkPermission($curUserID,"client_php_REMOVE_CLIENT",$connection);        
        if($checkPer==="1"){
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                if (isset($_POST['data'])) {    
                    if (isset($_POST['data']['user_id'])) {                        
                        ...
                        $ret_msg="Invalid Client/Customer ID";        
                        $return_data=array(
                            array(
                            "error_msg" => $ret_msg
                        ));
